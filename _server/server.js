...
const { PSPDFKIT_STANDALONE_LICENSE_KEY, SIGNING_SERVICE_URL } = process.env;

if (!PSPDFKIT_STANDALONE_LICENSE_KEY) {
  console.warn(
    "The PSPDFKIT_STANDALONE_LICENSE_KEY environment variable is not set. Examples will run in trial mode."
  );
}
...
app.prepare().then(() => {
  ...
  const handleExampleSessionEndpoint = async (
    ...
    try {
      ...
      const filePath = path.join(__dirname, "static", documentFile);
      const readStream = fs.createReadStream(filePath);

      await PSPDFKit.documents.upload(readStream, documentId, documentId);
    }
  ...
  server.post("/api/construction-plan", async (req, res) => {
    ...
    try {
      await PSPDFKit.documents.properties(CONSTRUCTION_PLAN_DOCUMENT_ID);
    } catch (_error) {
      // The document does not exist yet, we therefore upload it
      const filePath = path.join(__dirname, "static", "construction-plan.pdf");
      const readStream = fs.createReadStream(filePath);
  ...
  parsedExamples.forEach(({ name, fileSystemPath, fileInfo }) => {
    ...
    // have to re-upload the document. PSPDFKit Document Engine is smart enough to notice
    ...
    // means that PSPDFKit Document Engine was wiped or the document was manually
    ...
    server.post(`/server-document/${name}`, async (req, res) => {
      ...
      if (!documentId) {
        ...
        try {
          uploadResponse = await PSPDFKit.documents.upload(
            readStream,
            fileInfo.name
          );
      ...
      res.json({
        ...
        jwt: createJwt(documentId, layerName, jwtParameters),
        aidaJwt: createAIDocumentAssistantJwt(),
      });
    });
  ...
  server.get("/.well-known/apple-app-site-association", (req, res) => {
    res.json(require("./static/apple-app-site-association.json"));
  });
  ...
  server.get("/api/get-certificates", async (req, res) => {
    try {
      const response =
        req.query.backend === "api"
          ? await DocumentWebServicesAPI.signatures.getCertificates()
          : await PSPDFKit.signatures.getCertificates({
  ...
  server.post("/api/sign-hash", async (req, res) => {
    try {
      const params = {
        dataToBeSigned: req.body.dataToBeSigned,
        hash: req.body.hash,
        hashAlgorithm: req.body.hashAlgorithm || "sha256",
        signatureType: req.body.signatureType || "cades",
        cadesLevel: req.body.cadesLevel || "b-lt",
        ...(req.query.backend === "api"
          ? {}
          : { signingToken: req.body.signingToken || "user-1-with-rights" }),
      };
      ...
      const method =
        req.query.backend === "api"
          ? DocumentWebServicesAPI.signatures.signHash
          : PSPDFKit.signatures.signHash;
      ...
      console.error(error);
      res.status(500).send("Could not perform sign request.");
    }
  });
  ...
  server.post("/api/create-auth-token", async (req, res) => {
    try {
      ...
      const jwt = createJwtForProcessing({
        allowed_operations: allowedOperations,
        server_url:
          PSPDFKIT_SERVER_EXTERNAL_URL || PSPDFKIT_SERVER_INTERNAL_URL,
      });
