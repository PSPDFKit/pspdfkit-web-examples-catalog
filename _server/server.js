...
const BOARD_MEETING_DOCUMENT_ID = "board-meeting";
const CONSTRUCTION_PLAN_DOCUMENT_ID = "construction-plan";
const MARKETING_DEPT_SCHEDULE_DOCUMENT_ID = "marketing-department-schedule";
...
app.prepare().then(() => {
  ...
  server.post("/api/board-meeting", async (req, res) => {
    try {
      await PSPDFKit.documents.properties(BOARD_MEETING_DOCUMENT_ID);
    } catch (_error) {
      // The document does not exist yet, we therefore upload it
      const filePath = path.join(__dirname, "static", "board-meeting.pdf");
      const readStream = fs.createReadStream(filePath);

      await PSPDFKit.documents.upload(
        readStream,
        BOARD_MEETING_DOCUMENT_ID,
        BOARD_MEETING_DOCUMENT_ID
      );
    }

    const layerName = createLayerName();
    const shareableId = encodeShareableId(BOARD_MEETING_DOCUMENT_ID, layerName);

    res.header("Access-Control-Allow-Origin", "*");
    res.json({
      encodedDocumentId: shareableId,
      documentId: BOARD_MEETING_DOCUMENT_ID,
      jwt: createJwt(BOARD_MEETING_DOCUMENT_ID, layerName),
      url: `${req.protocol}://${req.get("host")}/custom?i=${shareableId}${
        req.params.inapp ? "&inapp=true" : ""
      }`,
      serverUrl: PSPDFKIT_SERVER_EXTERNAL_URL,
  ...
  server.post("/api/construction-plan", async (req, res) => {
    ...
    const layerNamesWithMetadata = layerNames.reduce((acc, layerName) => {
      ...
      const metadata = {
        ...
        jwt: createJwt(CONSTRUCTION_PLAN_DOCUMENT_ID, layerName),
        url: `${req.protocol}://${req.get("host")}/custom?i=${shareableId}${
          req.params.inapp ? "&inapp=true" : ""
        }`,
        serverUrl: PSPDFKIT_SERVER_EXTERNAL_URL,
  ...
  server.post("/api/marketing-department-schedule", async (req, res) => {
    try {
      await PSPDFKit.documents.properties(MARKETING_DEPT_SCHEDULE_DOCUMENT_ID);
    } catch (_error) {
      // The document does not exist yet, we therefore upload it
      const filePath = path.join(
        __dirname,
      ...
      await PSPDFKit.documents.upload(
        readStream,
        MARKETING_DEPT_SCHEDULE_DOCUMENT_ID,
        MARKETING_DEPT_SCHEDULE_DOCUMENT_ID
      );
    }
    ...
    res.json({
      encodedDocumentId: shareableId,
      documentId: MARKETING_DEPT_SCHEDULE_DOCUMENT_ID,
      jwt: createJwt(MARKETING_DEPT_SCHEDULE_DOCUMENT_ID, layerName),
      url: `${req.protocol}://${req.get("host")}/custom?i=${shareableId}${
        req.params.inapp ? "&inapp=true" : ""
      }`,
      serverUrl: PSPDFKIT_SERVER_EXTERNAL_URL,
  ...
  parsedExamples.forEach(({ name, fileSystemPath, fileInfo }) => {
    ...
    server.post(`/server-document/${name}`, async (req, res) => {
      ...
      if (!documentId) {
        ...
        let { document_id } = uploadResponse;
