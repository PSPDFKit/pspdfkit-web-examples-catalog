...
PSPDFKIT_SERVER_API_AUTH_TOKEN = process.env.PSPDFKIT_SERVER_API_AUTH_TOKEN || "secret";
...
if (!PSPDFKIT_SERVER_API_AUTH_TOKEN) {
  PS
  throw new Error(
    "You have to set the PSPDFKIT_SERVER_API_AUTH_TOKEN environment variable."
  );
}

/**
 * In this file, we're implementing the necessary PSPDFKit Document Engine to Server API.
 *
...
const PSPDFKit = {
  documents: {
     ...
     * passed. The documentId is the identifier on PSPDFKit Document Engine. We can pass
     * an arbitrary documentId.
     *
     * PSPDFKit Document Engine will extract the title of the document from the PDF
    ...
    async upload(readStream, filename, documentId = null) {
      ...
      const response = await fetch(
        `${PSPDFKIT_SERVER_INTERNAL_URL}/api/documents`,
        {
          method: "POST",
          headers: {
            Authorization: `Token token=${PSPDFKIT_SERVER_API_AUTH_TOKEN}`,
          },
          body: form,
        }
      ).then((res) => {
  ...
  signatures: {
    async signHash(signRequest) {
      const response = await fetch(
        `${PSPDFKIT_SERVER_INTERNAL_URL}/api/sign_hash`,
        {
          method: "POST",
          headers: {
            Authorization: `Token token=${PSPDFKIT_SERVER_API_AUTH_TOKEN}`,
    ...
    async getCertificates(request) {
      const response = await fetch(
        `${PSPDFKIT_SERVER_INTERNAL_URL}/api/get_certificates`,
        {
          method: "POST",
          headers: {
            Authorization: `Token token=${PSPDFKIT_SERVER_API_AUTH_TOKEN}`,
