 ...
 * those ids across different devices (to demonstrate Nutrient Instant) and
 * still be able to recover if the document is no longer valid. Additionally, we
 * are making our catalog public on https://www.nutrient.io/sdk and thus can't rely on
...
function createExampleHash(exampleName) {
  const sha256Buffer = crypto.createHash("sha256").update(exampleName).digest();
...
function decodeShareableId(id) {
  ...
  switch (segments.length) {
    ...
    default:
      throw new Error(
        `Sharable id error, got ${segments.length} segments. Please contact support@nutrient.io`
      );
  }

  if (!documentId && !layerName) {
    throw new Error(
      "Sharable id format error. Please contact support@nutrient.io"
    );
  }
