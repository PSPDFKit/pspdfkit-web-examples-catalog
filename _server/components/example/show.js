...
import { load, unload, validatePSPDFKitVersion } from "./utils";
...
export default class Example extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    // We only need to update the Nutrient Web SDK viewer when we switch between
    // different Web SDK backend options. All other updates can be
    // ignored.
    if (
  ...
  async componentDidMount() {
    ...
    try {
      this.setState({ showToolbarPreview: true });
      validatePSPDFKitVersion(PSPDFKit.version);
      const instance = await load(
        containerRef,
        name,
        currentBackend,
  ...
  render() {
    ...
    return (
      <Fragment>
        <Head>
          <title>{title} | Nutrient Web SDK Example</title>
        </Head>
