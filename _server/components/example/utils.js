  ...
  switch (backend) {
    case "server": {
      ...
      Object.assign(defaultConfiguration, getDefaultOptions("server"), {
        ...
        aiDocumentAssistant: aidaUrl ? {
          sessionId: "my-chat-session",
          jwt: serverDocument.aidaJwt,
          backendUrl: aidaUrl,
        } : undefined,
      });
      break;
...
function getServerUrl() {
  ...
  let serverUrl =
    PSPDFKIT_SERVER_URL_OR_PORT.indexOf("/") > 0
      ...
      : `${window.location.protocol}//${location.hostname}:${PSPDFKIT_SERVER_URL_OR_PORT}`;
...
function getAIDAUrl() {
  return document
    .querySelector('meta[name="pspdfkit-aida-url"]')
    .getAttribute("content");
}
