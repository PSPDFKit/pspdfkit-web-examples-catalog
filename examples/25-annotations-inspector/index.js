...
function getPropertiesByType(annotation) {
  ...
  const pushSectionForType = (annotationKey, label) => {
    ...
    sectionDiv.innerHTML = `<label class="group-title" for="${
      collapseBtn.id
    }">${label}</label>`;
...
// Amount of size difference between cloudyBorderInsetRect rect
// and the boundingBox rect. Used on ellipse annotations when
// specifying the cloudyBorderInsetRect property for them, so that
// its size is slightly less than the boundingBox, to make it
// fit nicely.
const CLOUDY_INSET_SIZE_DIFFERENCE = 16;
      ...
      getElements: () => {
        ...
        select.addEventListener("change", e => {
          ...
          if (e.target.selectedIndex === 5) {
            ...
            // but specify "cloudyBorderIntensity" and "cloudyBorderInsetRect"
            const updatedAnnotation = annotation
              ...
              .set(
                ...
                new PSPDFKit.Geometry.Rect({
                  top:
                    annotation.boundingBox.top +
                    CLOUDY_INSET_SIZE_DIFFERENCE / 2,
                  left:
                    annotation.boundingBox.left +
                    CLOUDY_INSET_SIZE_DIFFERENCE / 2,
                  width:
                    annotation.boundingBox.width - CLOUDY_INSET_SIZE_DIFFERENCE,
                  height:
                    annotation.boundingBox.height - CLOUDY_INSET_SIZE_DIFFERENCE
                })
              );
            instance.updateAnnotation(updatedAnnotation);
          } else if (annotation.cloudyBorderIntensity > 0) {
            const updatedAnnotation = annotation
              .set("cloudyBorderIntensity", 0)
              .set("cloudyBorderInsetRect", null);
            instance.updateAnnotation(updatedAnnotation);
          }
        });
        return [select];
...
function numberInput(prop, { min = 0, step, max, type = "number" }) {
  ...
  input.addEventListener(event, e => {
    ...
    const updatedAnnotation = instance.getSelectedAnnotation().set(prop, value);
    instance.updateAnnotation(updatedAnnotation);
    if (label) {
