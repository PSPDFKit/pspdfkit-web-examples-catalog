...
function duplicateAnnotationTooltipCallback(annotation) {
  ...
  const duplicateItem = {
    ...
    onPress: () => {
      ...
      // polygon annotations, as well as `cloudyBorderInsetRect` for annotations
      ...
      if (duplicatedAnnotation.cloudyBorderInsetRect) {
        duplicatedAnnotation = duplicatedAnnotation.set(
          "cloudyBorderInsetRect",
          duplicatedAnnotation.cloudyBorderInsetRect.translate(
            offsetTranslation
          )
        );
      }
