...
export async function load(defaultConfiguration) {
  ...
  const certificatesResponse = await fetch("/api/get-certificates", {
    method: "GET",
    headers: {
...
function createButtons() {
  return {
    ...
    resetButton: {
      ...
      name: "reset",
      async onPress() {
        location.href = "/digital-signatures-hash";
      },
    },
  };
}
...
async function signDocument(instance) {
  // Request the auth token from the backend.
  const authResponse = await fetch("/api/create-auth-token", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
  ...
  // Use it to perform the signing operation.
