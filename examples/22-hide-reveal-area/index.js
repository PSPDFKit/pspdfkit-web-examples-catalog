...
function createToolbarItems() {
  ...
  if (instance) {
    ...
    const currentPageHasRevealedArea = revealedAreasOnPageIndexes.has(
      currentPageIndex
    );

    const currentPageHasHiddenArea = hiddenAreasOnPageIndexes.has(
      currentPageIndex
    );
...
async function removeAnnotationsFromPage(pageIndex) {
  ...
  annotations
    ...
    .forEach((annotation) => instance.delete(annotation.id));
